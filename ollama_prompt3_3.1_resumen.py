...
# Columnas que se incluyen en el test SIN etiqueta
COLS_TEST_SIN_ETIQUETA = ['Tipo', 'Número', 'Añosanción', 'Título','resumen_ocr' ,'Resumen', 'Artículos']
FILE_TEST = 'test_sin_etiqueta_con_resumen.xlsx'
# Clasificador a usar:
...
def construir_prompt_usuario(row):
    ...
    resumen_ocr = str(row.get('resumen_ocr', '')).strip() if pd.notna(row.get('resumen_ocr')) else ''
    ...
    if resumen:
        texto += f"\nResumen: {resumen}"
    if resumen_ocr:
        texto += f"\nTexto completo: {resumen_ocr[:500]}"
    if articulos:
...
def main():
    ...
    # Guardar
    output = f'resultado_{CLASIFICADOR_ACTIVO}3.1_p3_resumen.xlsx'

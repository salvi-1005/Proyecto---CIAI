...
COLS_TEST_SIN_ETIQUETA = ['Tipo', 'Número', 'Añosanción', 'Título', 'Resumen', 'Artículos']
FILE_TEST = 'test_con_etiqueta_y_texto.xlsx'
# Clasificador a usar:
...
def construir_prompt_usuario(row):
    ...
    texto_pdf = str(row.get('texto_pdf', '')).strip() if pd.notna(row.get('texto_pdf')) else ''
...
def main():
    ...
    # Guardar
    output = f'resultado_{CLASIFICADOR_ACTIVO}3.1_p3_texto_completo.xlsx'
